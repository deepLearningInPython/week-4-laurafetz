...
def rnn_layer(w: np.array, list_of_sequences: list[np.array], sigma=sigmoid ) -> np.array:
    W = w[:9].reshape(3, 3)        # Extract and reshape W matrix (3x3)
    U = w[9:18].reshape(3, 3)      # Extract and reshape U matrix (3x3)
    B = w[18:21].reshape(1, 3)     # Extract and reshape B matrix (1x3)

    nr_sequences = len(list_of_sequences)
    outputs = np.empty(nr_sequences)  # Array to store outputs
    
    for i in range(nr_sequences):
        # Get i-th sequence
        X = list_of_sequences[i]
        # Initialize hidden state to 0 (same shape as a row of X)
